class ListsController < ApplicationController
  ...
  def index
    @movies_now = Movie.order('time DESC').limit(10).order("RAND()").limit(5)
    @movies_comming  = Movie.order('time DESC').limit(5)
    @movies_upcomming  = Movie.order('time DESC').limit(5)
    @movies_trend  = Movie.order("RAND()").limit(5)
    @countries = Country.all
    @genres = Genre.all
    @awards = Award.group(:name)
    @movies_age = Movie.group(:time).order('time DESC')
    @movies_rental = Movie.order('time DESC').limit(10).order("RAND()").limit(3)
  end
  def user
    # ↓フォロワー数で取得？
